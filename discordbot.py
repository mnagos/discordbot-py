    ...
    if users.bot == 1:
        ...
        try:
            ...
            if str(reaction.emoji) == '\U0001F4E5':
                ...
                await reaction.message.channel.send("플레이리스트가 나오면 생길 기능입니다.")
...
async def 재생(ctx, *, msg):
    ...
    try:
        ...
        try:
            ...
            await ctx.send("채널에 유저가 접속해있지않아용...") 
    ...
    if not vc.is_playing():
        ...
        chromedriver_dir = r"C:\Users\a7502\Downloads\chromedriver_win32\chromedriver.exe"
        driver = webdriver.Chrome(chromedriver_dir, options = options)
        driver.get("https://www.youtube.com/results?search_query="+msg+"+lyrics")
        source = driver.page_source
        bs = bs4.BeautifulSoup(source, 'lxml')
...
async def 나가(ctx):
    try:
        ...
        await ctx.send("이미 그 채널에 속해있지않아요")    
